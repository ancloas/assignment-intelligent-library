...
class DatabaseManager:
    ...
    async def fetch_all(self, query, params=None):
        result_rows = await self.execute(query, params)
        result = [row._asdict() for row in result_rows.fetchall()]
        return result
    

    async def fetch_one(self, query, params=None):
        execute_result = await self.execute(query, params)
        result = execute_result.fetchone()
        if result:
